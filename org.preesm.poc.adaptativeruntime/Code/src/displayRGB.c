...
void displayRGB(int id, int height, int width, unsigned char *rgb){
	...
	for (idxPxl = 0; idxPxl < h*w; idxPxl++){
		*(((char*)(surface->pixels)) + 4 * idxPxl + 0) = *(rgb + idxPxl + 2*height*width);
		*(((char*)(surface->pixels)) + 4 * idxPxl + 1) = *(rgb + idxPxl + height*width);
		*(((char*)(surface->pixels)) + 4 * idxPxl + 2) = *(rgb + idxPxl );
	}
