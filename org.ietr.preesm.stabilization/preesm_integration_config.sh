...
function preesm_project_build () {
  (mkdir -p Code/bin && cd Code/bin && rm -rf * && cmake -D CMAKE_C_FLAGS="-DPREESM_VERBOSE -DPREESM_LOOP_SIZE=300 " .. && make -j4)
  return $?
}

function preesm_project_exec () {
  (cd Code/ && xvfb-run -a --server-args="-ac -screen 0 1280x1024x16" ./bin/Release/stabilization)
  return $?
}
