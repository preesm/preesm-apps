...
int build_gaussian_pyramid(
  ...
  // write_float_pgm(ff, floatImg->data, floatImg->w, floatImg->h,1);
#if SIFT_IMG_DBL
  upsample_2x<float>(octaves[0], floatImg);
  // char ff2x[256];
  // sprintf(ff2x, "up2x.pgm");
  // write_float_pgm(ff2x, octaves[0]->data, octaves[0]->w, octaves[0]->h,1);
  release(octaves[1]);
  octaves[1] = floatImg;
#else
  release(octaves[0]);
...
int build_gaussian_pyramid_original(
  ...
  //write_pgm<unsigned char>(firstImage, "ohbaoh.pgm");
...
int sift_cpu(
  ...
  for (int i = 0; i < nOctaves; i ++){
    for (int j = 0; j < nDogLayers; j ++){
      ...
      write_float_pgm(fdog, dogPyr[i*nDogLayers+j]->data, dogPyr[i*nDogLayers+j]->w, dogPyr[i*nDogLayers+j]->h, 1);
    }
  }
#endif
  ...
  // for (int i = 0; i < nOctaves; i ++) {
  //   for (int j = 1; j < 4; j ++) {
  //     sprintf(frot, "ang-%d-%d.pgm", i, j);
  //     fprintf(stderr, "w: %d\t h: %d\n", rotPyr[i*nGpyrLayers+j]->w, rotPyr[i*nGpyrLayers+j]->h);
  //     write_float_pgm(frot, rotPyr[i*nGpyrLayers+j]->data, rotPyr[i*nGpyrLayers+j]->w, rotPyr[i*nGpyrLayers+j]->h, 2);
  //   }
  // }
