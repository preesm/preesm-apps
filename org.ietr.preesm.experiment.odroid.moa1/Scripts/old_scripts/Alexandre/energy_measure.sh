...
if [ $1 == "A7" ]; then
	while [ ! -z "$(pidof $2)" ]; do
		...
		CPU_W_A7=`cat /sys/bus/i2c/drivers/INA231/2-0045/sensor_W`
		CPU_A_A7=`cat /sys/bus/i2c/drivers/INA231/2-0045/sensor_A`
		CPU_V_A7=`cat /sys/bus/i2c/drivers/INA231/2-0045/sensor_V`
	...
	while [ ! -z "$(pidof $2)" ]; do
		...
		#Get the values of the sensors
		CPU_W_A15=`cat /sys/bus/i2c/drivers/INA231/2-0040/sensor_W`
		CPU_A_A15=`cat /sys/bus/i2c/drivers/INA231/2-0040/sensor_A`
		CPU_V_A15=`cat /sys/bus/i2c/drivers/INA231/2-0040/sensor_V`
	...
	while [ ! -z "$(pidof $2)" ]; do
		...
		CPU_W_A7=`cat /sys/bus/i2c/drivers/INA231/2-0045/sensor_W`
		CPU_W_A15=`cat /sys/bus/i2c/drivers/INA231/2-0040/sensor_W`
		CPU_W_ME=`cat /sys/bus/i2c/drivers/INA231/2-0041/sensor_W`
	
		CPU_A_A7=`cat /sys/bus/i2c/drivers/INA231/2-0045/sensor_A`
		CPU_A_A15=`cat /sys/bus/i2c/drivers/INA231/2-0040/sensor_A`
		CPU_A_ME=`cat /sys/bus/i2c/drivers/INA231/2-0041/sensor_A`

		CPU_V_A7=`cat /sys/bus/i2c/drivers/INA231/2-0045/sensor_V`
		CPU_V_A15=`cat /sys/bus/i2c/drivers/INA231/2-0040/sensor_V`
		CPU_V_ME=`cat /sys/bus/i2c/drivers/INA231/2-0041/sensor_V`
		...
		#Write there values in tables
		tab_W[$measurement_number]=`printf "$CPU_W_A7, $CPU_W_A15, $CPU_W_ME \n "`
		tab_A[$measurement_number]=`printf "$CPU_A_A7, $CPU_A_A15, $CPU_A_ME \n "`
		tab_V[$measurement_number]=`printf "$CPU_V_A7, $CPU_V_A15, $CPU_V_ME \n "`
...
echo "${tab_W[@]}" >> stats/power.csv;
echo "${tab_A[@]}" >> stats/intensity.csv;
echo "${tab_V[@]}" >> stats/tension.csv;

if [ $1 == "A15" -o $1 == "All" ];then
	echo "${tab_T_5[@]}" >> stats/temp_core_5.csv;
	echo "${tab_T_6[@]}" >> stats/temp_core_6.csv;
	echo "${tab_T_7[@]}" >> stats/temp_core_7.csv;
	echo "${tab_T_8[@]}" >> stats/temp_core_8.csv;
fi
