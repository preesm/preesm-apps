...
void setNumberOfUpdate(IN const float *delta, IN float *variance, OUT float *updateVariance, Param *N) {
    ...
    if (first) {
        first = false;
        variance[0] = VARIANCE_INIT_VAL;
    }
    (*N) = 40;
//    if (delta[0] < 0.f) {
//        (*N) = static_cast<Param>(std::ceil(-1.f * delta[0] / std::sqrt(variance[0])));
//        fprintf(stderr, "N value:%ld\n", *N);
//    } else {
//        (*N) = 0;
//    }
    updateVariance[0] = variance[0] * (1 - VARIANCE_BETA_RATE) + VARIANCE_BETA_RATE * delta[0] * delta[0];
}
